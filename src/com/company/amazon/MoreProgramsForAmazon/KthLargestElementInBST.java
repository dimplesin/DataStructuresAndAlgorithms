package com.company.amazon.MoreProgramsForAmazon;

public class KthLargestElementInBST {
	// C++ program to find k'th largest element in BST
	 
	
	// A utility function to create a new BST node
	BTreeNode newNode(int item)
	{
	    BTreeNode temp = new BTreeNode();
	    temp.data = item;
	    temp.left = temp.right = null;
	    return temp;
	}
	 
	// A function to find k'th largest element in a given tree.
	void kthLargestUtil(BTreeNode root, int k, int c)
	{
	    // Base cases, the second condition is important to
	    // avoid unnecessary recursive calls
	    if (root == null || c >= k)
	        return;
	 
	    // Follow reverse inorder traversal so that the
	    // largest element is visited first
	    kthLargestUtil(root.right, k, c);
	 
	    // Increment count of visited BTreeNodes
	    c++;
	 
	    // If c becomes k now, then this is the k'th largest 
	    if (c == k)
	    {
	        System.out.println( "K'th largest element is "
	             + root.data);
	        return;
	    }
	 
	    // Recur for left subtree
	    kthLargestUtil(root.left, k, c);
	}
	 
	// Function to find k'th largest element
	void kthLargest(BTreeNode root, int k)
	{
	    // Initialize count of BTreeNodes visited as 0
	    int c = 0;
	 
	    // Note that c is passed by reference
	    kthLargestUtil(root, k, c);
	}
	 
	/* A utility function to insert a new BTreeNode with given data in BST */
	BTreeNode insert(BTreeNode node, int data)
	{
	    /* If the tree is empty, return a new node */
	    if (node == null) return newNode(data);
	 
	    /* Otherwise, recur down the tree */
	    if (data < node.data)
	        node.left  = insert(node.left, data);
	    else if (data > node.data)
	        node.right = insert(node.right, data);
	 
	    /* return the (unchanged) node pointer */
	    return node;
	}
	 
	// Driver Program to test above functions
	int main()
	{
	    /* Let us create following BST
	              50
	           /     \
	          30      70
	         /  \    /  \
	       20   40  60   80 */
	    BTreeNode root = null;
	    root = insert(root, 50);
	    insert(root, 30);
	    insert(root, 20);
	    insert(root, 40);
	    insert(root, 70);
	    insert(root, 60);
	    insert(root, 80);
	 
	    int c = 0;
	    for (int k=1; k<=7; k++)
	        kthLargest(root, k);
	 
	    return 0;
	}
}
